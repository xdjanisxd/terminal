...
- Parser-independent printable-character, carriage-return, line-feed, index, reverse-index, next-line, backspace, horizontal-tab, full-screen-scroll, and typed rendition operations routed through `TerminalState`
...
- Parser routing for printable input, CR, LF, BS, HT, HTS, IND, RI, NEL, TBC, CUU/CUD/CUF/CUB, CUP/HVP, CHA/VPA, ED, EL, full-screen SU/SD, IRM, DECAWM, DECTCEM, and SGR styles, defaults, ANSI 16 colors, and semicolon-form indexed foreground/background colors; unsupported parser actions and parameters are ignored without approximation
...
- One hundred sixty-three `terminal-core` unit/integration tests plus three compile-fail ownership tests
...
- The parser recognizes broader VTE syntax incrementally, but only printable input, CR, LF, BS, HT, HTS, IND, RI, NEL, the supported cursor/erase/full-screen-scroll CSI subset, IRM/DECAWM/DECTCEM, and the documented style, default, ANSI 16-color, and semicolon-form indexed-color SGR subset currently have terminal semantics
