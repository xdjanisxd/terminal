...
- Parser routing for printable input, CR, LF, BS, CUU/CUD/CUF/CUB, CUP/HVP, CHA/VPA, ED, and EL; unsupported parser actions are ignored without approximation
...
- Ninety-four `terminal-core` unit/integration tests plus three compile-fail ownership tests
...
- The parser recognizes broader VTE syntax incrementally, but only printable input, CR, LF, BS, and the supported cursor/erase CSI subset currently have terminal semantics
...
- Remaining CSI/SGR semantics, mode dispatch, OSC/DCS semantics, horizontal tab stops, terminal replies, scrolling margins, saved cursor state, origin mode, and alternate screens
