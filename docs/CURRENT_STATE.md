...
- Parser routing for printable input, CR, LF, BS, HT, HTS, IND, RI, NEL, primary DA1 queries `CSI c`/`CSI 0 c`, ANSI cursor-position DSR `CSI 6 n`, TBC, CUU/CUD/CUF/CUB, CUP/HVP, CHA/VPA, ED, EL, region-aware SU/SD, DECSTBM, IRM, private DECCKM/DECAWM/DECTCEM, and SGR styles, defaults, ANSI 16 colors, semicolon-form indexed foreground/background colors, and semicolon-form exact RGB foreground/background colors; unsupported parser actions and parameters are ignored without approximation
...
- Two hundred forty-five `terminal-core` unit/integration tests plus four compile-fail ownership tests
