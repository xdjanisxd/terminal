...
- Controlled state operations for cursor movement, full-screen scroll up/down, horizontal tabs and tab-stop mutation, current rendition, clearing, resizing, supported mode changes, and a project-owned reset
...
- Delayed right-margin wrapping, auto-wrap suppression, bounded full-screen SU/SD, fixed-screen bottom scrolling, and fixed-width insert/replace output behavior
- Incremental project-owned `TerminalParser` adapter encapsulating `vte` and preserving parser state across input chunks
- Parser routing for printable input, CR, LF, BS, HT, HTS, TBC, CUU/CUD/CUF/CUB, CUP/HVP, CHA/VPA, ED, EL, full-screen SU/SD, IRM, DECAWM, DECTCEM, and SGR styles, defaults, ANSI 16 colors, and semicolon-form indexed foreground/background colors; unsupported parser actions and parameters are ignored without approximation
...
- One hundred fifty-three `terminal-core` unit/integration tests plus three compile-fail ownership tests
...
- The parser recognizes broader VTE syntax incrementally, but only printable input, CR, LF, BS, horizontal-tab operations, the supported cursor/erase/full-screen-scroll CSI subset, IRM/DECAWM/DECTCEM, and the documented style, default, ANSI 16-color, and semicolon-form indexed-color SGR subset currently have terminal semantics
