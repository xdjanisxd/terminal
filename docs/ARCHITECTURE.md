...
- Full-screen scrolling remains a bounded `ScreenGrid` cell movement owned by `TerminalState`. Counts are clamped to the visible height before calculating one overlapping copy; counts at or above the height clear the grid. Moved cells retain complete attributes, newly exposed rows use `Cell::default()`, and cursor, modes, current rendition, and delayed wrap are preserved. SU/SD/IND/RI/NEL intentionally do not consult active scrolling margins in this foundation slice.
