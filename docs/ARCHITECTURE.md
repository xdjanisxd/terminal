...
- `TerminalState` privately owns a `ScreenSet` of independent primary and alternate `ScreenGrid` buffers, Primary-only bounded scrollback, current rendition, `TerminalModes`, `InputModes`, horizontal tab-stop state bounded by the validated column count, and a fixed-capacity FIFO of pending project-owned terminal replies.
...
- The current project-owned reset preserves dimensions and already-generated pending replies, recreates both blank screen buffers with Primary active and uninitialized saved-cursor slots, restores current rendition and supported modes to defaults, restores horizontal tab stops every eight columns beginning at zero-based column 8, and restores full-screen vertical scrolling margins. Resize also preserves pending replies. This reset is not DECSTR or RIS.
...
- `ScreenGrid` owns a distinct bounded inclusive region-scroll primitive. It receives explicit validated row bounds, remains independent of `TerminalState`, active-margin ownership, and scrollback, clamps counts to region height before calculating one overlapping copy, clears only the selected region at or above that height, moves complete `Cell` values without allocation, and fills newly exposed rows with `Cell::default()`. `ScreenState` owns the higher-level upward-scroll capture path: it copies rows before invoking the existing grid primitive only when Primary scrolls the full visible screen upward. `TerminalState` translates active `VerticalScrollingMargins` into explicit grid regions for IND, RI, SU, and SD. NEL composes the same region-aware index operation with carriage return, so it inherits bounded movement and region scrolling without separate boundary logic. Cursor visibility, modes, current rendition, tab stops, and margins are preserved; SU/SD preserve delayed wrap, while IND, RI, and NEL cancel it.
- `TerminalState` owns two independent project `ScreenState` buffers selected by typed `ScreenKind`. Each buffer owns its grid/cursor, vertical scrolling margins, delayed-wrap state, and one optional `SavedCursor` slot. Only Primary additionally owns a `Scrollback`; Alternate has no history storage. Scrollback holds at most 10,000 exact `Vec<Cell>` rows in chronological order, evicting the oldest row before retaining a new row at capacity. It receives complete rows only when Primary's whole visible screen moves upward: bottom-edge full-screen line feed/IND, NEL through that index path, delayed-wrap output which invokes that path, and explicit full-screen SU. Restricted DECSTBM scrolling, IL/DL, RI/SD and other downward scrolling, erase/edit operations, and every Alternate operation never append. Full reset clears Primary history; `?47`, `?1047`, and `?1049` switching preserves it. Rows retain capture-time widths across resize with no crop, padding, reflow, or rewrapping, so later renderer work must handle mixed historical widths. Immutable chronological inspection is available through `TerminalState::scrollback_len` and `scrollback_row`; mutable storage and viewport state remain private.
...
`terminal-pty` owns the project-owned local-session contract and its concrete `PortablePtyBackend` adapter. `PtySpawnConfig` owns an executable path, exact platform arguments, optional working directory, environment pairs, and a validated nonzero `PtySize`; it intentionally supplies no shell policy. `PortablePtyBackend` converts that request to `portable-pty`'s native PTY system, command builder, and character-cell size while keeping every backend type out of public project signatures. `PortablePtySession` owns the master PTY, child handle, writer, single output-reader capability, and lifecycle state.

`PtyOutputReader` transfers exclusive ownership of the one raw PTY reader. It returns arbitrary byte chunks without UTF-8 decoding, normalization, terminal parsing, or message-boundary assumptions; a `read` result of zero is raw output EOF. EOF is intentionally independent from `PtyLifecycle::Exited(PtyExitStatus)`: either observation may reach the application first. A second `take_output_reader` call returns `PtyError::NotRunning` and never creates a competing reader. Output-read failures map to the project-owned `PtyError::ReadFailed`; no `portable-pty`, `anyhow`, or `std::io` error type escapes.

`PtyWorker` is the project-owned bounded runtime boundary. Its controller owns a bounded command sender, bounded event receiver, and deterministic join handle. One session thread owns the `PtySession`, lifecycle polling, and `Write(Vec<u8>)`/`Resize(PtySize)`/`Terminate` commands; one dedicated reader thread owns the exclusive `PtyOutputReader`. The reader's blocking `read` cannot starve commands. `std::sync::mpsc::sync_channel` supplies the only queues: eight owned commands and eight events. The reader reads at most 1,024 raw bytes per event, so a full event queue bounds queued raw output to eight chunks and blocks its producer rather than dropping bytes, EOF, exit, or errors. The session thread polls lifecycle at a bounded 20 ms interval without busy spinning. `PtyWorkerEvent::Output(PtyOutput::{Bytes,Eof,Exited})` preserves arbitrary chunking; empty byte events are never emitted; EOF and exit are each emitted at most once with no ordering guarantee. Worker failures use `PtyWorkerError` and never expose channel, I/O, or backend types.

On natural exit, the session thread emits `Exited` once and releases the adapter master through lifecycle polling; the reader emits `Eof` once when its raw read returns zero, and the session thread joins the reader after both terminal observations. Explicit termination reaches `PtySession::terminate`, rejects later queued write/resize work with a project error event, then continues lifecycle/read handling until shutdown. Controller command-channel loss terminates the session to avoid an orphan child. Event-receiver loss makes sends fail rather than block forever, requests termination, and joins both threads. `shutdown_and_join` intentionally discards unread events, requests termination, and joins; dropping the controller follows the same deterministic policy. Parser, terminal state, renderer, input encoding, shell policy, tabs, panes, and application event-loop code remain outside this boundary.
...
`terminal-workspace` owns tab order, pane identity, session association, focused tab/pane, a recursive split tree, and per-tab temporary zoom state. A workspace definition is a declarative startup recipe with tab titles, split axes, local-shell leaves, and focus indexes; zoom is not part of that recipe. `terminal-app` owns each pane's live parser, terminal state, and PTY worker, routes input to the focused pane, and continues draining background output. Closing a pane drops its worker and removes its layout leaf; the final pane is retained. The renderer displays every pane rectangle supplied by the workspace; while zoomed, only the focused pane fills the viewport.
