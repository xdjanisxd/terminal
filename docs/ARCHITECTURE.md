...
- `TerminalState` privately owns one active `ScreenGrid`, current rendition, `TerminalModes`, `InputModes`, horizontal tab-stop state bounded by the validated column count, inclusive zero-based `VerticalScrollingMargins` bounded by the validated row count, and a fixed-capacity FIFO of pending project-owned terminal replies.
...
- The current project-owned reset preserves dimensions and already-generated pending replies, clears the active screen, homes the cursor, restores current rendition and supported modes to defaults, restores horizontal tab stops every eight columns beginning at zero-based column 8, and restores full-screen vertical scrolling margins. Resize also preserves pending replies. This reset is not DECSTR or RIS.
...
- A private `vte::Perform` implementation translates printable characters, CR, LF, BS, HT, HTS, IND, RI, NEL, primary DA1, ANSI DSR cursor-position queries, and the supported cursor/erase/scroll/margin/mode/TBC/SGR CSI subset into `TerminalState` semantic methods only. It never mutates grids, cell attributes, tab-stop storage, scrolling-margin storage, reply storage, or mode models directly, and it performs no host or PTY I/O. DECCKM selects the typed input-related cursor-key mode only; a future input encoder consumes that state, so this parser path emits no key bytes.
- The supported CSI subset includes primary DA queries `CSI c` and `CSI 0 c`, ANSI DSR cursor-position query `CSI 6 n`, CUU, CUD, CUF, CUB, CUP/HVP, CHA, VPA, bounded current-row ICH, ED modes 0-2, EL modes 0-2, cursor-relative IL/DL, region-aware SU/SD, DECSTBM, TBC current/all, standard IRM, private DECCKM/DECAWM/DECTCEM, and SGR reset, bold, italic, underline, inverse, ANSI standard/bright, `38;5;n`/`48;5;n` indexed, and semicolon-form `38;2;r;g;b`/`48;2;r;g;b` exact RGB foreground/background colors, plus channel-default restoration. IL/DL use a `TerminalState`-owned subregion from the current cursor row through the active bottom margin, and are no-ops outside the active margins; their parser path only normalizes the single CSI count and invokes that semantic method. ICH uses a `TerminalState`-owned current-row operation that delegates bounded right-shifting and default-blank insertion to `ScreenGrid`; it preserves cursor coordinates while cancelling delayed wrap. Nonzero, extra, subparameter, private-prefixed, secondary, and tertiary DA forms remain safe no-ops. CSI parameters and numeric protocol identifiers remain private to the adapter; cursor and scroll operations normalize omitted and zero parameters, and cursor and DECSTBM operations convert one-based coordinates to project-owned zero-based semantics. DECSTBM defaults omitted or zero top to row 1 and omitted or zero bottom to the screen height, rejects equal or reversed multi-row bounds, out-of-range values, extra scalar parameters, and subparameter groups atomically, and permits the full one-row region as the single-row invariant exception.
