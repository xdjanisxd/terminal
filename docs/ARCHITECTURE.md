...
- The supported CSI subset includes primary DA queries `CSI c` and `CSI 0 c`, ANSI DSR cursor-position query `CSI 6 n`, CUU, CUD, CUF, CUB, CUP/HVP, CHA, VPA, ED modes 0-2, EL modes 0-2, cursor-relative IL/DL, region-aware SU/SD, DECSTBM, TBC current/all, standard IRM, private DECCKM/DECAWM/DECTCEM, and SGR reset, bold, italic, underline, inverse, ANSI standard/bright, `38;5;n`/`48;5;n` indexed, and semicolon-form `38;2;r;g;b`/`48;2;r;g;b` exact RGB foreground/background colors, plus channel-default restoration. IL/DL use a `TerminalState`-owned subregion from the current cursor row through the active bottom margin, and are no-ops outside the active margins; their parser path only normalizes the single CSI count and invokes that semantic method. Nonzero, extra, subparameter, private-prefixed, secondary, and tertiary DA forms remain safe no-ops. CSI parameters and numeric protocol identifiers remain private to the adapter; cursor and scroll operations normalize omitted and zero parameters, and cursor and DECSTBM operations convert one-based coordinates to project-owned zero-based semantics. DECSTBM defaults omitted or zero top to row 1 and omitted or zero bottom to the screen height, rejects equal or reversed multi-row bounds, out-of-range values, extra scalar parameters, and subparameter groups atomically, and permits the full one-row region as the single-row invariant exception.
