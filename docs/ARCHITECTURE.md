...
- `TerminalState` privately owns one active `ScreenGrid`, `TerminalModes`, `InputModes`, and horizontal tab-stop state bounded by the validated column count.
- Callers receive immutable screen and mode access. Printable output, carriage return, line feed, backspace, horizontal-tab movement and tab-stop mutation, typed cursor movement, typed erase, clearing, resizing, and mode transitions use semantic methods on `TerminalState`.
...
- The current project-owned reset preserves dimensions, clears the active screen, homes the cursor, restores supported modes to defaults, and restores horizontal tab stops every eight columns beginning at zero-based column 8. It is not DECSTR or RIS.
...
- A private `vte::Perform` implementation translates printable characters, CR, LF, BS, HT, HTS, and the supported cursor/erase/mode/TBC CSI subset into `TerminalState` semantic methods only. It never mutates grids, tab-stop storage, or mode models directly.
- The supported CSI subset is CUU, CUD, CUF, CUB, CUP/HVP, CHA, VPA, ED modes 0-2, EL modes 0-2, TBC current/all, standard IRM, and private DECAWM and DECTCEM. CSI parameters and numeric protocol identifiers remain private to the adapter; cursor operations normalize omitted and zero parameters and convert one-based absolute coordinates to project-owned zero-based semantics. Unsupported and malformed mode parameters are ignored individually without preventing supported parameters in the same syntactically valid sequence from taking effect.
