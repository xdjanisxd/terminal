...
- `TerminalState` owns two independent project `ScreenState` buffers selected by typed `ScreenKind`. Each buffer owns its grid/cursor, vertical scrolling margins, delayed-wrap state, and one optional `SavedCursor` slot. A saved slot stores only cursor coordinates and current rendition; it never owns cells, tab stops, margins, terminal/input modes, replies, or delayed-wrap. `save_cursor` replaces the active slot without other mutation; `restore_cursor` is a no-op before save, restores only the active slot, clamps coordinates at restore against current dimensions, restores rendition, and cancels delayed wrap. Current rendition remains shared except when an active saved slot explicitly restores its captured value. `?47` does not alter slots; `?1047` entry recreates Alternate (therefore clears its slot), and exit selects Primary without restore. `?1049 h` from Primary saves that Primary slot, resets Alternate with the established `?1047` policy, and selects Alternate; repeated entry while Alternate is active is a no-op. `?1049 l` selects Primary then restores only its saved slot. Switching is atomic and does not copy or merge cells; resize applies existing grid semantics to both buffers and leaves saved coordinates unmodified for clamp-on-restore; reset recreates both blank buffers with Primary active.
