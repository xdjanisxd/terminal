...
`PtyOutputReader` transfers exclusive ownership of the one raw PTY reader. It returns arbitrary byte chunks without UTF-8 decoding, normalization, terminal parsing, or message-boundary assumptions; a `read` result of zero is raw output EOF. EOF is intentionally independent from `PtyLifecycle::Exited(PtyExitStatus)`: exit can be observed before buffered output is drained, and EOF can be observed independently by the future output worker. `PtyOutput::Bytes`, `PtyOutput::Eof`, and `PtyOutput::Exited` remain future worker event forms; this adapter supplies only their I/O and lifecycle primitives. A second `take_output_reader` call returns `PtyError::NotRunning` and never creates a competing reader. Output-read failures map to the project-owned `PtyError::ReadFailed`; no `portable-pty`, `anyhow`, or `std::io` error type escapes.

Only the session owner writes, resizes, terminates, and consumes output. Writes and character-cell resize succeed only while lifecycle is `Running`; lifecycle polling persists a known child exit, releases the master PTY once exit is known so the exclusive reader can drain buffered bytes and observe EOF, and then rejects operations with `PtyError::NotRunning`. The verified transitions are `Running -> Exited` on natural exit and `Running -> TerminationRequested -> Exited` after termination; a rapid backend exit may be observed as `Exited` immediately after the termination request. `terminate` changes lifecycle before dropping the writer and requesting backend termination, so later operations cannot race ahead; repeated termination is idempotent. `Drop` remains a backend safety net, not lifecycle control. The reader stays independently usable through exit for buffered output and EOF. `portable-pty` is isolated to `terminal-pty`; there is no parser, `TerminalState`, renderer, input encoder, worker, queue, or shell-policy coupling in this slice.
