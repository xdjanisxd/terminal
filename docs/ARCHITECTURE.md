...
- Callers receive immutable screen, rendition, and mode access. Printable output, carriage return, line feed, backspace, full-screen scroll up/down, horizontal-tab movement and tab-stop mutation, typed cursor movement, typed erase, rendition changes, clearing, resizing, and mode transitions use semantic methods on `TerminalState`.
...
- Full-screen scrolling is a bounded `ScreenGrid` cell movement owned by `TerminalState`. Counts are clamped to the visible height before calculating one overlapping copy; counts at or above the height clear the grid. Moved cells retain complete attributes, newly exposed rows use `Cell::default()`, and cursor, modes, current rendition, and delayed wrap are preserved. No scrolling-region or scrollback model is implied.
...
- A private `vte::Perform` implementation translates printable characters, CR, LF, BS, HT, HTS, and the supported cursor/erase/scroll/mode/TBC/SGR CSI subset into `TerminalState` semantic methods only. It never mutates grids, cell attributes, tab-stop storage, or mode models directly.
- The supported CSI subset is CUU, CUD, CUF, CUB, CUP/HVP, CHA, VPA, ED modes 0-2, EL modes 0-2, full-screen SU/SD, TBC current/all, standard IRM, private DECAWM and DECTCEM, and SGR reset, bold, italic, underline, inverse, ANSI standard/bright and `38;5;n`/`48;5;n` indexed foreground/background colors, and channel-default restoration. CSI parameters and numeric protocol identifiers remain private to the adapter; cursor and scroll operations normalize omitted and zero parameters, and cursor operations convert one-based absolute coordinates to project-owned zero-based semantics.
