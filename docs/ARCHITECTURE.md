...
- Callers receive immutable screen, rendition, and mode access. Printable output, carriage return, line feed, index/reverse-index/next-line controls, backspace, full-screen scroll up/down, horizontal-tab movement and tab-stop mutation, typed cursor movement, typed erase, rendition changes, clearing, resizing, and mode transitions use semantic methods on `TerminalState`.
...
- A private `vte::Perform` implementation translates printable characters, CR, LF, BS, HT, HTS, IND, RI, NEL, and the supported cursor/erase/scroll/mode/TBC/SGR CSI subset into `TerminalState` semantic methods only. It never mutates grids, cell attributes, tab-stop storage, or mode models directly.
