...
- A private `vte::Perform` implementation translates printable characters, CR, LF, BS, HT, HTS, IND, RI, NEL, primary DA1, ANSI DSR cursor-position queries, and the supported cursor/erase/scroll/margin/mode/TBC/SGR CSI subset into `TerminalState` semantic methods only. It never mutates grids, cell attributes, tab-stop storage, scrolling-margin storage, reply storage, or mode models directly, and it performs no host or PTY I/O.
