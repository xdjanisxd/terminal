...
Only the session owner writes, resizes, terminates, and consumes output. Writes and character-cell resize succeed only while lifecycle is `Running`; lifecycle polling persists a known child exit and then rejects those operations with `PtyError::NotRunning`. `terminate` first changes lifecycle to `TerminationRequested`, drops the writer, then requests backend termination, so later operations cannot race ahead; repeated termination is idempotent. `Drop` remains a backend safety net, not lifecycle control. `portable-pty` is isolated to `terminal-pty`; there is no parser, `TerminalState`, renderer, input encoder, worker, queue, or shell-policy coupling in this slice.
