...
- `TerminalState` privately owns one active `ScreenGrid`, current rendition, `TerminalModes`, `InputModes`, and horizontal tab-stop state bounded by the validated column count.
- Callers receive immutable screen, rendition, and mode access. Printable output, carriage return, line feed, backspace, horizontal-tab movement and tab-stop mutation, typed cursor movement, typed erase, rendition changes, clearing, resizing, and mode transitions use semantic methods on `TerminalState`.
...
- The current project-owned reset preserves dimensions, clears the active screen, homes the cursor, restores current rendition and supported modes to defaults, and restores horizontal tab stops every eight columns beginning at zero-based column 8. It is not DECSTR or RIS.
...
- Printing uses VT-style delayed wrapping: writing the final column leaves the cursor there and records a private pending wrap; the next printable character wraps before it is written when auto-wrap is enabled. Disabling auto-wrap keeps output at the final column. Typed cursor movement, horizontal tab, carriage return, erase, clearing, resize, and reset cancel a pending wrap. Line feed preserves pending wrap as well as the cursor column; backspace cancels it only when the cursor can move left.
- `CellAttributes` is both the typed current-rendition value and the immutable snapshot stored with each printed cell. It separates normal/bold intensity, upright/italic style, underline, inverse video, foreground, and background without parser identifiers. Color values reuse `CellColor`; this slice maps only default and ANSI 16-color indices.
...
- A private `vte::Perform` implementation translates printable characters, CR, LF, BS, HT, HTS, and the supported cursor/erase/mode/TBC/SGR CSI subset into `TerminalState` semantic methods only. It never mutates grids, cell attributes, tab-stop storage, or mode models directly.
- The supported CSI subset is CUU, CUD, CUF, CUB, CUP/HVP, CHA, VPA, ED modes 0-2, EL modes 0-2, TBC current/all, standard IRM, private DECAWM and DECTCEM, and SGR reset, bold, italic, underline, inverse, ANSI standard/bright foreground/background, and channel-default restoration. CSI parameters and numeric protocol identifiers remain private to the adapter; cursor operations normalize omitted and zero parameters and convert one-based absolute coordinates to project-owned zero-based semantics.
- SGR parameters are applied in order. Bare SGR and empty parameters are reset operations, matching `vte` parameter dispatch and terminal convention. Unsupported scalar parameters and subparameter groups are ignored individually, so supported neighbors in the same bounded parser callback still apply without approximating 256-color, truecolor, or unsupported styles.
