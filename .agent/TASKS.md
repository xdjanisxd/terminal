...
- [ ] Add the next M1 slice: integrate a minimal incremental `vte` adapter behind a project-owned interface and route printable-character behavior through `TerminalState`. Test arbitrary input chunking and keep CSI, SGR, PTY, rendering, and other terminal behavior out of that slice.
